...
Strategy: ramp up slowly (+1A per cycle), ramp down immediately if over limit.
...
def _run():
    ...
    # Max safe current for 3-phase (bottleneck phase)
    max_safe = max(CHARGER_MIN_A, min(CHARGER_MAX_A, int(head.mn())))
    ...
    log.info("EV: 3ph %dA→%dA (safe=%d desired=%d deadline=%d) SOC=%.0f%% price=%.2f | head L1=%.1f L2=%.1f L3=%.1f",
